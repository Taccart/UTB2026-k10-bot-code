...
- [IsServiceInterface](IsServiceInterface.md) - Base service interface
- [IsOpenAPIInterface](IsOpenAPIInterface.md) - OpenAPI service interface
...
private:
    ...
    // Helper methods (AsyncWebServerRequest* passed via lambdas)
...
bool TemperatureAlertService::registerRoutes()
{
    ...
    // POST /api/tempalert/v1/threshold - Set alert threshold
    {
        ...
        webserver.on(path.c_str(), HTTP_POST, [this](AsyncWebServerRequest *request) {
            this->handleSetThreshold(request);
        });
    }
    ...
    // POST /api/tempalert/v1/enable - Enable/disable alerts
    {
        ...
        webserver.on(path.c_str(), HTTP_POST, [this](AsyncWebServerRequest *request) {
            this->handleEnableAlert(request);
        });
    }
