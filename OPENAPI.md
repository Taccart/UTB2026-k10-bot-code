...
## Rolling Logger Service

### GET /api/logs/v1
Retrieve log entries from a specified logger instance in either JSON or plain text format.

**Tags:** Logs  
**Authentication:** None

**Query Parameters:**
- `logger` (string, required) - Logger name: "debug", "info", or "esp"
- `format` (string, optional) - Response format: "json" or "log" (default: "json")

**Response Codes:**
- `200` - Logs retrieved successfully
- `422` - Missing or invalid logger parameter
- `423` - Logger not defined or service not started
...
```json
{
  ...
  "entries": [
    ...
    "UDP service listening on port 5005"
  ]
}
```

**Response Format (format=log):**
```
[DEBUG] WiFi Service initialized
[DEBUG] HTTP server started on port 80
[DEBUG] UDP service listening on port 5005
```

**Available Loggers:**
- `debug` - Verbose debug logger with detailed service messages
- `info` - Application-level information logger for user-facing messages  
- `esp` - ESP32 system logger integration

**Note:** Logs are stored in circular buffers. Historical log size depends on configuration.
...
All services that implement `IsOpenAPIInterface` automatically provide these standard endpoints:

### GET /api/<service>/<version>/serviceStatus
...
- `initialize failed` - Initialization failed
- `start failed` - Service failed to start
- `stop failed` - Service failed to stop cleanly

### GET /api/<service>/<version>/saveSettings
Save service-specific settings to persistent storage (NVS/EEPROM).
...
### GET /api/<service>/<version>/loadSettings
Load service-specific settings from persistent storage.

**Tags:** Varies by service  
**Authentication:** None

**Response Codes:**
- `200` - Settings loaded successfully
...
- `423 Locked` - Service not started or required resource not available
