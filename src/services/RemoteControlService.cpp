...
bool RemoteControlService::initializeService()
{
    ...
    {
        logger->error(getServiceName() + ": missing required services");
        return false;
    }
...
bool RemoteControlService::startService()
{
    ...
    if (handler_id < 0)
    {
        logger->error(getServiceName() + ": failed to register UDP handler");
        return false;
    }
...
bool RemoteControlService::handleMessage(const std::string& message, const IPAddress& remoteIP, uint16_t remotePort)
{
    ...
    if (cmd == fpstr_to_string(FPSTR(RemoteControlConsts::cmd_forward)))
    {
        executeForward();
        #ifdef VERBOSE_DEBUG
        logger->debug("Remote control: forward command from " + remoteIP.toString().c_str());
        #endif
        return true;
    }
    else if (cmd == fpstr_to_string(FPSTR(RemoteControlConsts::cmd_backward)))
    {
        executeBackward();
        #ifdef VERBOSE_DEBUG
        logger->debug("Remote control: backward command from " + remoteIP.toString().c_str());
        #endif
        return true;
    }
    else if (cmd == fpstr_to_string(FPSTR(RemoteControlConsts::cmd_turn_left)))
    {
        executeTurnLeft();
        #ifdef VERBOSE_DEBUG
        logger->debug("Remote control: turn_left command from " + remoteIP.toString().c_str());
        #endif
        return true;
    }
    else if (cmd == fpstr_to_string(FPSTR(RemoteControlConsts::cmd_turn_right)))
    {
        executeTurnRight();
        #ifdef VERBOSE_DEBUG
        logger->debug("Remote control: turn_right command from " + remoteIP.toString().c_str());
        #endif
        return true;
    }
    else if (cmd == fpstr_to_string(FPSTR(RemoteControlConsts::cmd_stop)))
    {
        executeStop();
        #ifdef VERBOSE_DEBUG
        logger->debug("Remote control: stop command from " + remoteIP.toString().c_str());
        #endif
        return true;
    }
...
bool RemoteControlService::saveSettings()
{
    // To be implemented if needed
    return true;
}

bool RemoteControlService::loadSettings()
{
    // To be implemented if needed
    return true;
}
