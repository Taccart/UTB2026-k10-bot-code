...
<body>
  ...
  // API endpoint for master registration - PUT to set master IP and generate token
    ...
    if (master_IP.length() > 0) {
      ...
      DEBUG_TO_SERIAL("Master already registered - showing conflict dialog");
  ...
  //       DEBUG_TO_SERIAL("  PPM sent successfully");
  //     } else {
  //       DEBUG_TO_SERIAL("  Invalid frame received");
  //       server->send(500, "text/plain", "Invalid camera frame");
  //     }
  //   } else {
  //     DEBUG_TO_SERIAL("  Timeout waiting for frame");
  ...
  //     } else {
  //       DEBUG_TO_SERIAL("  Invalid frame");
  //       server->send(500, "text/plain", "Invalid frame");
  //     }
  //   } else {
  //     DEBUG_TO_SERIAL("  Timeout");
