...
void DFR0548_Controller::setPWMFreq(float freq)
{
    ...
    if (prescaleval < 3)
        prescaleval = 3; // Min value
    if (prescaleval > 255)
        prescaleval = 255; // Max value
...
{
    if (channel >= DFR0548_MAX_CHANNELS)
        return;
...
{
    if (channel >= DFR0548_MAX_CHANNELS || _servos[channel].type != SERVO_TYPE_ANGULAR)
        return;
...
{
    if (channel >= DFR0548_MAX_CHANNELS || _servos[channel].type != SERVO_TYPE_CONTINUOUS)
        return;
...
std::string DFR0548_Controller::getChannelStatus(uint8_t channel)
{
    if (channel >= DFR0548_MAX_CHANNELS)
        return "Invalid channel";

    std::string status = "Channel " + std::to_string(channel) + ": ";
    if (_servos[channel].type == SERVO_TYPE_ANGULAR)
    {
        status += "Angular, Angle: " + std::to_string(_servos[channel].currentAngle) + "°";
    }
    else
    {
        status += "Continuous, Speed: " + std::to_string(_servos[channel].currentSpeed) + "%";
    }
    status += ", Range: " + std::to_string(_servos[channel].maxAngle) + "°";
    return status;
}
