...
void setup() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (ServoHandler_init()) {
      unihiker.canvas->canvasText("Servo: Started", line++, 0x00FF00);
  DEBUG_TO_SERIAL("Servo controller initialized successfully");

      // Register servo routes with the web server
      WebServerModule_registerServo(&server);
    } else {
      unihiker.canvas->canvasText("Servo: Init Failed", line++, 0xFF0000);
  DEBUG_TO_SERIAL("ERROR: Failed to initialize servo controller");
    ...
    // Camera capture task on Core 0 (priority 2) - capture frames continuously
    //xTaskCreatePinnedToCore(camera_capture_task, "Camera_Task", 4096, NULL, 2, NULL, 0);
